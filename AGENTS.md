...
- `ddp_env()` recorta el espacio exterior de cada variable. Sin eso, un `DB_USER` pegado como `" root"` hacia que MySQL respondiera `1045 Access denied for user ' root'` con la contrasena correcta.
- El autoload de `public/index.php` resuelve los directorios sin distinguir mayusculas: el namespace es `App\Controllers\Admin` pero la carpeta es `app/controllers/admin`. En Windows no se nota; en Linux el panel entero devolvia 500.
- El resolvedor recorre los segmentos del namespace con `is_dir` y, si falla, `strcasecmp` sobre `scandir`. El nombre del archivo si se resuelve con la caja del namespace.
- `Database` encadena intentos `tls` -> `public-key` -> `plain` porque MySQL 8/9 usa `caching_sha2_password`. El perfil de conexion se registra en el log, nunca el valor de la contrasena.
