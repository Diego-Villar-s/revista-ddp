...
class Podcast extends Model
{
    ...
    public function getUltimos(int $limit = 4): array
    {
        $limit = max(1, $limit);
        return $this->db->select(
            "SELECT * FROM podcasts
    ...
    public function paginateAdmin(array $filters = [], int $page = 1, int $perPage = ITEMS_PER_PAGE): array
    {
        ...
        $total = $this->db->count("SELECT COUNT(*) FROM podcasts p {$whereClause}", $params);
        $offset = ($page - 1) * $perPage;
        $items = $this->db->select(
            "SELECT p.*, u.nombres AS usuario_nombres, u.ap_paterno AS usuario_ap
             FROM podcasts p
